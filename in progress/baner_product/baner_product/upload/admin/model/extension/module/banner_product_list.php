...
class ModelExtensionModuleBannerProductList extends Model {
	...
	public function addBanner($data) {
		...
		if (isset($data['product'])) {
			foreach ($data['product'] as $id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "banner_product_product SET banner_id = '" . (int)$banner_id . "', language_id = '" . (int)$language_id . "', product_id = '" . (int)$id . "'");
			}
		}
	...
	public function getBannerProducts($banner_id) {
		...
		foreach ($banner_product_query->rows as $banner_product) {
			$banner_product_data[$banner_product['language_id']][] = array(
				'product'      => $banner_product['product_id']
			);
		}
