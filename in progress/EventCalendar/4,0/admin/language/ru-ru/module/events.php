<?php
// Heading
$_['heading_title']				= 'События';

// Text
$_['text_success']				= 'Успех: вы изменили события!';
$_['text_success_view']			= 'Успешно изменено %s событий!';
$_['text_list']					= 'Список событий';
$_['text_add']					= 'Добавить событие';
$_['text_edit']					= 'Редактировать событие';
$_['text_plus']					= '+';
$_['text_minus']				= '-';
$_['text_license']				= 'Для получения ключа, отправьте сообщение на e-meil: <b>gen2023@yandex.ru</b>, <b>genodessa@gmail.com</b>';
$_['text_events_setting']		= 'Глобальные настройки события';
$_['text_events_setting_update']		= 'Редактирование настроек события';
$_['text_editNameRightMenu'] = 'Изменить названия пунктов правого меню';
$_['text_repeat_day']			= 'Повторять раз в день';
$_['text_repeat_nex_day']		= 'Повторять через день';
$_['text_repeat_week']			= 'Повторять раз в неделю';
$_['text_repeat_month']			= 'Повторять раз в месяц';
$_['text_repeat_year']			= 'Повторять раз в год';
$_['text_repeat_no']			= 'Не повторять';
$_['text_keyword'] = 'Не используйте пробелы, замените пробелы на - и убедитесь, что URL-адрес SEO глобально уникален.';

//таб
$_['tab_license']				= 'Лицензия';
$_['tab_data']					= 'Данные';
$_['tab_calendar']				= 'Настройки календаря';
$_['tab_editNameRightMenu']   = 'Изменить названия';
$_['tab_seo']				= 'SEO';

// Колонки
$_['column_image']				= 'Изображение';
$_['column_title']				= 'Название события';
$_['column_date_to']			= 'Дата начала события';
$_['column_date_from']			= 'Дата конца события';
$_['column_status']				= 'Статус';
$_['column_action']				= 'Действие';

// Вводимые значения
$_['entry_title']				= 'Введите название';
$_['entry_image']				= 'Изображение';
$_['entry_description']			= 'Описание';
$_['entry_mindescription']		= 'Короткое описание';
$_['entry_date_to']				= 'Дата начала';
$_['entry_date_from']			= 'Дата окончания';
$_['entry_time_to']          = 'Время начала';
$_['entry_time_from']        = 'Время окончания';
$_['entry_status']				= 'Статус';
$_['entry_sort_order']			= 'Сортировка события';
$_['entry_license']				= 'Введите ключ';
$_['entry_events_url']			= 'SEO URL календаря:';
$_['entry_eventslist_url']		= 'SEO URL списка событий:';
$_['entry_eventsDetail_url']	= 'SEO URL описания события:';
$_['entry_store']				= 'Магазины:';
$_['entry_meta_title']			= 'HTML-тег Title';
$_['entry_meta_h1']				= 'HTML-тег H1';
$_['entry_meta_keyword']		= 'Мета-тег Keywords:';
$_['entry_meta_description']	= 'Мета-тег Description:';
$_['entry_keyword']				= 'Введите SEO URL';
$_['entry_alternativeLink']		= 'Введите альтернативную сылку';
$_['entry_product']				= 'Товар';
$_['entry_event_color']			= 'Цвет события';
$_['entry_firstDay']			= 'Выберите начало недели';
$_['entry_dayMaxEvents']		= 'Введите ограничение кол-во событий на 1 день';
$_['entry_initialView']	 	 	= 'Выберите главное отображение:';
$_['entry_list_rightMenu'] 	 	= 'Выберите отображение с правого меню:';
$_['entry_repeat'] 	 			= 'Повтороение события';

//Дни недели
$_['Monday']					= 'Понедельник';
$_['Tuesday']					= 'Вторник';
$_['Wednesday']					= 'Среда';
$_['Thursday']					= 'Четверг';
$_['Friday']					= 'Пятница';
$_['Saturday']					= 'Суббота';
$_['Sunday']					= 'Воскресенье';

//list right panel
$_['dayGridMonth'] 	= 'Месяц';
$_['timeGridWeek']	= 'Неделя по времени';
$_['timeGridDay']	= 'День по времени';
$_['listYear'] 		= 'Список на год';
$_['listMonth'] 	= 'Список на месяц';
$_['listDay'] 		= 'Список на день';
$_['listWeek'] 		= 'Список на неделю';

// Помощь
$_['help_keyword']				= 'Используйте тире вместо пробелов. Должно быть уникально на всю систему';
$_['help_product']				= '(Автодополнение)';
$_['help_event_color']			= 'Неиспользуйте цвет белый. Можно водить RGB или HEX (пример: black или #000 или #000000)';
$_['help_dayMaxEvents']			= 'Числом';

// Помощь
$_['help_keyword']				= 'Используйте тире вместо пробелов. Должно быть уникально на всю систему';
$_['help_product']				= '(Автодополнение)';

// Ошибка
$_['error_date']				= 'Дата окончания события должна быть такой же или после даты начала события';
$_['error_warning']				= 'Предупреждение: пожалуйста, внимательно проверьте форму на наличие ошибок!';
$_['error_permission']			= 'Предупреждение: у вас нет прав на изменение событий!';
$_['error_title']				= 'Cобытие должно быть больше 3 и меньше 255 символов!';
$_['error_mindescription']		= 'Cобытие должно быть больше 3 и меньше 255 символов!';
$_['error_description']			= 'Cобытие должно быть больше 3 и меньше 255 символов!';
$_['error_license']				= 'Введите ключ активации!';
$_['error_setting_viewRightMenu']= 'Введите Пункты отопражения правого меню!';
$_['error_license_setting']		= 'Для установки ключа, нажмите кнопку настроек!';
$_['error_keyword']				= 'Этот SEO keyword уже используется!';

$_['button_setting']			= 'Настройки';
$_['button_hideEvent']			= 'Отключить прошедшие события';