...
class ControllerExtensionModuleEventList extends Controller {
	...
	public function index(){
			...
			foreach ($events_list as $result) {
				...
				$data['events_list'][] = array(
					...
					'description' => utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES,
						'UTF-8')), 0, $event_setting['description_limit']),
		...
		$data['sorts'][] = array(
			...
			'value' => 'id.title-ASC',
			'href' => $this->url->link('extension/module/event_list', 'sort=id.title&order=ASC' . $url)
		);

		$data['sorts'][] = array(
			'text' => $this->language->get('text_title_desc'),
			'value' => 'id.title-DESC',
			'href' => $this->url->link('extension/module/event_list', 'sort=id.title&order=DESC' . $url)
		);
		...
		($events_total) ? (($page - 1) * $limit) + 1 : 0, ((($page - 1) * $limit) > ($events_total - $limit)) ? $events_total : ((($page - 1) * $limit) + $limit), $events_total, ceil($events_total / $limit));

		// http://googlewebmastercentral.blogspot.com/2011/09/pagination-with-relnext-and-relprev.html
		if ($page == 1) {
			$this->document->addLink($this->url->link('extension/module/event_list'), 'canonical');
		} else {
			$this->document->addLink($this->url->link('extension/module/event_list', '&page='. $page), 'canonical');
		}
		
		if ($page > 1) {
			    $this->document->addLink($this->url->link('extension/module/event_list', (($page - 2) ? '&page='. ($page - 1) : '')), 'prev');
			}

		if ($limit && ceil($events_total / $limit) > $page) {
			$this->document->addLink($this->url->link('extension/module/event_list', '&page='. ($page + 1)), 'next');
		}
