...
//var_dump($events_list);				
		if ($events_list) {
			...
			foreach ($events_list as $result) {
				...
				$data['events_list'][] = array(
					...
					'event_title1' 		=> $result['title1'],
					'thumb' 			=> $image,
					'event_description' => utf8_substr(strip_tags(html_entity_decode($result['event_description'], ENT_QUOTES,
					...
					'doctor_title' 		=> $result['doctor_title'],
					'href_doctor'		=> $this->url->link('extension/module/doctor_detail', '&doctor_id=' . $result['doctor_id']),
					'price'				=> $price
				);
			}
		...
		$data['sorts'][] = array(
			'text' => $this->language->get('text_doctor_desc'),
			'value' => 'd.title-DESC',
			'href' => $this->url->link('extension/module/event_list', 'sort=d.title&order=DESC' . $url)
		);	
		
		$data['sorts'][] = array(
			'text' => $this->language->get('text_doctor_asc'),
			'value' => 'd.title-ASC',
			'href' => $this->url->link('extension/module/event_list', 'sort=d.title&order=ASC' . $url)
		);
