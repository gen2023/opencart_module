...
class ModelExtensionModuleEvents extends Model {
	...
	public function install() {
		...
		$query1=$this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_url` WHERE `query`='extension/module/event_detail'");
		$num_rows1 = $query1->num_rows;
		if ($num_rows1==0)	{
			$this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url` SET `query` = 'extension/module/event_detail', `keyword` = 'event_detail'");
		}
	...
	public function setEventsSetting($data) {
		...
		if ($data) {
			...
			//var_dump($data);
			$this->db->query("UPDATE " . DB_PREFIX . "gen_events_setting SET firstDay = '" . (int)$data['event_firstDay'] . "', dayMaxEvents = '" . (int)$data['dayMaxEvents'] . "', rightColumnMenu = '" . $this->db->escape($rightColumnMenu) . "', initialView = '" . $this->db->escape($event_initialView) . "', value = '" . $this->db->escape($data['value']) . "' WHERE event_value_id = '1'");
