...
class ControllerExtensionModuleEvents extends Controller {
	...
	public function setting() {	
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateSetting($val)) {
			...
			$this->model_extension_module_events->deleteModule('events');

			if ($this->request->post['slider']==1){
				$this->model_extension_module_events->addModule('events','events_slider','1');
			} else{
				$this->model_extension_module_events->addModule('events','events_slider','0');
			}

			if ($this->request->post['calendar']==1){
				$this->model_extension_module_events->addModule('events','events_calendar','1');
			} else{
				$this->model_extension_module_events->addModule('events','events_calendar','0');
			}
		...
		$data['text_sliderEvents'] = $this->language->get('text_sliderEvents');
		...
		$data['entry_sliderEvents'] = $this->language->get('entry_sliderEvents');
		...
		$data['tab_sliderEvents'] = $this->language->get('tab_sliderEvents');
