...
class ModelExtensionModuleEvents extends Model {
	...
	public function uninstall() { 
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "module` WHERE `code` = 'events'");
	}
	...
	public function setEventsSetting($data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "gen_events_setting SET 
			width = '" . (int)$data['width'] . "', 
			height = '" . (int)$data['height'] . "', 
			slider = '" . (int)$data['slider'] . "', 
			event_share = '" . (int)$data['event_share'] . "', 
			firstDay = '" . (int)$data['event_firstDay'] . "', 
			dayMaxEvents = '" . (int)$data['dayMaxEvents'] . "', 
			rightColumnMenu = '" . $this->db->escape($rightColumnMenu) . "', 
			initialView = '" . $this->db->escape($event_initialView) . "', 
			value = '" . $this->db->escape($data['value']) . "' WHERE event_value_id = '1'");
	...
	public function addModule($code,$name,$status) {
		$result=["status"=>"$status"];
		$result=json_encode($result);

		if ($name=='events_slider'){$name='Events slider';}
		if ($name=='events_calendar'){$name='Events calendar';}
		$this->db->query("INSERT INTO `" . DB_PREFIX . "module` SET `name` = '".$name."', `code` = '" . $this->db->escape($code) . "', `setting` = '" . $result . "'");
	}
