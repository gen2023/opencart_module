...
class ControllerExtensionFeedSimpleYMLGoogle extends Controller {
	...
	protected function getProducts() {
		...
		foreach ($products as $product) {
			$productsOptionSize = $this->model_extension_feed_simple_yml->getOption($product['product_id'],'Size');
			//var_dump($productsOptionSize[0]);
		$productsOptionColor = $this->model_extension_feed_simple_yml->getOption($product['product_id'],'Color');

		(count($productsOptionColor)>2) ? $newcolor=array_slice($productsOptionColor, 0, 3) : $newcolor=$productsOptionColor;
			$output .= '<item>';
			...
			$output .= '<g:brand>Ivory and Kate</g:brand>';
			$output .= '<g:size>'.$productsOptionSize[0].'</g:size>';
			$output .= '<g:age_group>'. $product['age_group'].'</g:age_group>';
			
			$output .= '<g:color>'.implode("/",$newcolor).'</g:color>';
			$output .= '<g:condition>new</g:condition>';
			$output .= '<g:gender>male</g:gender>';
			...
			$output .= '<g:google_product_category>2271</g:google_product_category>';
			$output .= '</item>';
		}
