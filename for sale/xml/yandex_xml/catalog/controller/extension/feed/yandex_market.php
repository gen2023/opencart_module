...
class ControllerExtensionFeedYandexMarket extends Controller {
	...
	public function index() {
		if ($this->config->get('yandex_market_status')) {
			...
			$supported_currencies = array('RUR', 'RUB', 'BYR', 'BYN', 'KZT', 'UAH', 'USD', 'EUR');
...
*/
			...
			$fp = fopen("feed_yandex.xml", "w");
			fwrite($fp, $this->getYml());
			fclose($fp);
	...
	private function setCurrency($id, $rate = 'CBRF', $plus = 0) {
		$allow_id = array('RUR', 'RUB', 'BYR', 'BYN', 'KZT', 'UAH', 'USD', 'EUR');
	...
	private function setOffer($data) {
		...
		$allowed_tags = array('url'=>0, 'buyurl'=>0, 'price'=>1, 'wprice'=>0, 'currencyId'=>1, /* 'quantity'=>1, */ 'xCategory'=>0, 'categoryId'=>1, 'pictures'=>0, 'store'=>0, 'pickup'=>0, 'delivery'=>0, 'deliveryIncluded'=>0, 'local_delivery_cost'=>0, 'orderingTime'=>0);
		...
		$allowed_tags = array_merge($allowed_tags, array('aliases'=>0, 'additional'=>0, 'description'=>0, 'sales_notes'=>0, 'promo'=>0, 'manufacturer_warranty'=>0, 'country_of_origin'=>0, 'downloadable'=>0, 'adult'=>0, 'barcode'=>0, 'code'=>1));
