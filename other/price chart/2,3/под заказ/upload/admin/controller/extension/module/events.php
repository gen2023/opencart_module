...
class ControllerExtensionModuleEvents extends Controller {
	...
	private function getList() {
    	...
    	foreach ($results as $result) {
			...
			$data['events'][] = array(
				...
				'status'     	=> $this->comparisonDate($result['event_id'],$result['status'],$result['date_from']),
	...
	private function getForm() { 
		...
		$data['entry_title1'] = $this->language->get('entry_title1');
		...
		$data['entry_event_price'] = $this->language->get('entry_event_price');
		$data['entry_event_doctor'] = $this->language->get('entry_event_doctor');
		...
		if (isset($this->request->post['title1'])) {
			...
			$data['title1'] = $this->model_extension_module_events->getEventsDescriptions($this->request->get['event_id']);
		} else {
			$data['title1'] = array();
		}
		...
		if (isset($this->request->post['title3'])) {
			...
			$data['title3'] = $this->model_extension_module_events->getEventsDescriptions($this->request->get['event_id']);
		} else {
			$data['title3'] = array();
		}
		
		if (isset($this->request->post['additional_field'])) {
			$data['additional_field'] = $this->request->post['additional_field'];
		} elseif (isset($events_info['additional_field'])) {
			$data['additional_field'] = $events_info['additional_field'];
		} else {
		...
		if (isset($this->request->post['price'])) {
			$data['event_price'] = $this->request->post['price'];
		} elseif (!empty($events_info)) {
			$data['event_price'] = $events_info['price'];
		} else {
			$data['event_price'] = '';
		}
		...
		if (!empty($this->request->post['doctor'])) {
			$doctor = $this->request->post['doctor'];
		} elseif (!empty($events_info['doctor_id'])) {			
			$doctor = $events_info['doctor_id'];
		} else {
			$doctor = array();
		}
		
		if (!empty($events_info['doctor_id'])){
			$doctor_info = $this->model_extension_module_events->getDoctor($events_info['doctor_id']);
	...
	public function setting() {	
		...
		$seo_event=$setting[0];
		$seo_eventlist=$setting[1];
		$seo_eventDetail=$setting[2];
		$event_share=$setting[3];
		$event_firstDays=$setting[4];
		$dayMaxEvents=$setting[5];
		
		$data['days']=[1=>$data['Monday'],2=>$data['Tuesday'],3=>$data['Wednesday'],4=>$data['Thursday'],5=>$data['Friday'],6=>$data['Saturday'],7=>$data['Sunday']];
		
		if($event_share['event_share']){
			$data['event_share'] = $event_share['event_share'];
		}else{
			$data['event_share'] = '';
		}
		...
		if($seo_eventDetail['keyword']){
			$data['eventsDetail_url'] = $seo_eventDetail['keyword'];
		}else{
			$data['eventsDetail_url'] = '';
		}
		
		if($event_firstDays){
			$data['event_firstDays'] = $event_firstDays;
		}else{
			$data['event_firstDays'] = 1;
		}
		
		if($dayMaxEvents['dayMaxEvents']){
			$data['dayMaxEvents'] = $dayMaxEvents["dayMaxEvents"];
		}else{
			$data['dayMaxEvents'] = '0';
		}
...
//var_dump($results);
	...
	public function autocompleteDoctors() {
		$json = array();
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_extension_module_events->getDoctors($filter_data);
//var_dump($results);
			foreach ($results as $result) {
	...
	protected function comparisonDate($event_id, $status,$dateFrom) {
		$this->load->model('extension/module/events');

		if (strtotime(date('d.m.Y'))>strtotime($dateFrom)) {
			$status=0;
			$editStatusEvent = $this->model_extension_module_events->editStatusEvent($event_id, $status);
			};
